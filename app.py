...
with st.sidebar:
    ...
    st.info("**สะมาริตันส์:** 02-113-6789")
    st.divider()
...
if not api_key:
    st.error("⚠️ ไม่พบ GEMINI_API_KEY ใน App Settings > Secrets")
    st.stop()
...
if prompt:
    ...
    with st.chat_message("assistant"):
        if is_crisis:
            ans = "เรารู้สึกเป็นห่วงคุณมากๆ โทรหาผู้เชี่ยวชาญได้ที่ 1323 (ฟรี 24 ชม.) หรือ 02-113-6789 นะครับ"
            st.error("⚠️ แจ้งเตือนความปลอดภัย")
            st.markdown(ans)
        else:
            try:
                # สร้าง client สดใหม่ต่อคำขอเพื่อป้องกัน Session หลุด
